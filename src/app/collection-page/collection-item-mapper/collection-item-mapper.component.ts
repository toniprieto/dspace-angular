...
export class CollectionItemMapperComponent implements OnInit {
    ...
    this.mappedItemsRD$ = collectionAndOptions$.pipe(
      switchMap(([collectionRD, options, shouldUpdate]) => {
        ...
        return this.itemDataService
          .findEditAuthorized(options.query, this.buildFilterQuery(collectionRD.payload.id, null), findOptions, !shouldUpdate, false, followLink('owningCollection'));
      }),
    );
  }
   ...
   */
  buildFilterQuery(collectionId: string, query: string): string {
