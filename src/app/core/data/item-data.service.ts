...
export abstract class BaseItemDataService extends IdentifiableDataService<Item> implements CreateData<Item>, PatchData<Item>, DeleteData<Item> {
   ...
   *    item list
   */
  public findEditAuthorized(query: string, filterQuery: string, options: FindListOptions, useCachedVersionIfAvailable = true, reRequestOnStale = true, ...linksToFollow: FollowLinkConfig<Item>[]): Observable<RemoteData<PaginatedList<Item>>> {
    options = { ...options, searchParams: [new RequestParam('query', query), new RequestParam('filterQuery', filterQuery)] };
